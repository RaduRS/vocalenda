...
const UK_LOCALE_OPTIONS = {
  locale: enGB,
  weekStartsOn: 1 as const, // Monday = 1
};

/**
 * Get the current date and time in UK timezone
 */
export function getCurrentUKDateTime(): Date {
...
/**
 * Format a date in UK long format (e.g., "Monday, 6 September 2025")
 */
export function formatUKDateLong(date: Date): string {
  return format(date, 'EEEE, d MMMM yyyy', UK_LOCALE_OPTIONS);
}
 ...
 * @param date - Date object to format
 * @returns Natural language date (e.g., "Wednesday, the 13th", "Monday, the 21st")
 */
export function formatConversationalDate(date: Date): string {
 ...
 * @param date - Date object to format
 * @returns Natural language date with month (e.g., "Wednesday, the 13th of September")
 */
export function formatConversationalDateWithMonth(date: Date): string {
  const dayName = format(date, 'EEEE', UK_LOCALE_OPTIONS);
  const dayNumber = date.getDate();
  const ordinalSuffix = getOrdinalSuffix(dayNumber);
 ...
 */
export function parseUKTime(timeString: string, baseDate?: Date): Date {
 ...
 */
export function parseUKDateTime(datetimeString: string): Date {
 ...
 */
export function convertToUKTime(date: Date, fromTimezone: string): Date {
 ...
 */
export function convertFromUKTime(date: Date, toTimezone: string): Date {
 ...
 */
export function createUKDateTime(isoDate: string, timeString: string): Date {
 ...
 * @param timezone - Target timezone (defaults to UK)
 * @returns ISO string with timezone info
 */
export function createTimezoneAwareISO(date: Date, timezone: string = UK_TIMEZONE): string {
 ...
 */
export function getStartOfWeekUK(date: Date): Date {
 ...
 */
export function getWeekDaysUK(): string[] {
 ...
 */
export function isValidUKDate(dateString: string): boolean {
  try {
 ...
 */
export function isValidUKTime(timeString: string): boolean {
  try {
 ...
 * @param time12h - Time in 12-hour format (e.g., "3:00 PM", "11:30 AM")
 * @returns Time in 24-hour format (e.g., "15:00", "11:30")
 */
 ...
 * @param time24h - Time in 24-hour format (e.g., "15:00", "09:30")
 * @returns Time in 12-hour format (e.g., "3:00 PM", "9:30 AM")
 */
export function convert24to12Hour(time24h: string): string {
 ...
 * @param baseDate - The date to compare against (defaults to current UK time)
 * @returns Human-readable description (e.g., "today", "tomorrow", "Monday", "next week")
 */
export function getRelativeTimeDescription(date: Date, baseDate?: Date): string {
  const base = baseDate || getCurrentUKDate();
...
// Export commonly used date-fns functions with UK locale defaults
